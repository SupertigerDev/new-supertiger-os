    ...
    await new Promise((resolve, reject) => {
      ...
      const processFileRequestIter = prevRequestResult => {
        ...
        if (nextFileRequest.done) {
          resolve();
          return;
        } else if (!(nextFileRequest.value instanceof self.DOMRequest ||
                     nextFileRequest.value instanceof self.IDBRequest)) {
...
export class IDBFileStorage {
  ...
  initializedDB() {
    ...
    this.initializedPromise = (async () => {
      if (self.IDBMutableFile && this.persistent) {
  ...
  async put(fileName, file) {
    ...
    if (!(file instanceof File) && !(file instanceof Blob) &&
        !(self.IDBMutableFile && file instanceof self.IDBMutableFile) &&
  ...
  async get(fileName) {
    ...
    return waitForDOMRequest(objectStore.get(fileName)).then(result => {
      if (self.IDBMutableFile && result instanceof self.IDBMutableFile) {
        return new IDBPromisedMutableFile({
          filesStorage: this,
          idb,
          fileName,
